...
@section('content')
  ...
  @include(
         ...
         array(
            'menu' => array(
                'male and female show all' => URL::to('tag/pattern/all/all'),
                'male and plus size blogger' => URL::to('tag/pattern/male/yes'),
                'male and not plus size blogger' =>  URL::to('tag/pattern/male/no'),
                'male and not and plus size blogger' => URL::to('tag/pattern/male/both'),
                'female and plus size blogger' => URL::to('tag/pattern/female/yes'),
                'female and not plus size blogger' => URL::to('tag/pattern/female/no'),
                'female and both not and plus size blogger' => URL::to('tag/pattern/female/both'),
                'male and female plus size blogger' => URL::to('tag/pattern/both/yes'),
                'male and female not plus size blogger' => URL::to('tag/pattern/both/no'),
                'male and female both plus size blogger' => URL::to('tag/pattern/both/both'),
            )
         )
      )

     {!! Form::open(array('route' => array('tag.pattern.update', 2)   , 'method' => 'PUT', 'enctype'=>'multipart/form-data')) !!}

      <div class="portlet portlet-default">

        <div class="portlet-header">
          <h3 class="portlet-title"><u>Pattern</u></h3>
        </div> <!-- /.portlet-header -->
                ...
                @foreach ($pagination as $page)
                    ...
                    <td>
                      <div class="thumbnail">
                        <div class="thumbnail-view">
                          <a class="thumbnail-view-hover ui-lightbox" href="{{Session::get('imgSrcUploads') . '/pattern/' . $page->id . '.jpg'}}">
                            <i class="fa fa-plus" ></i>
                          </a>
                          <img src="{{Session::get('imgSrcUploads') . '/pattern/' . $page->id . '.jpg' }}" width="125" alt="Gallery Image" />
                        </div>
                  ...
                  <td class="valign-middle">
                    ...
                    <td class="text-center valign-middle">
                      ...
                      &nbsp;
                      <span class="btn btn-xs btn-primary" onclick="admin_tag_delete_item('#garment-item-container-{{$page->id}}', '{{$page->id}}', 'pattern', '{{route('tag.pattern.destroy', $page->id)}}')" ><i class="fa fa-times"></i></span>
