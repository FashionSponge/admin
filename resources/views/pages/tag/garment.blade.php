...
@section('content')
  <div class="container">
      ...
      @include(
         ...
         array(
            'menu' => array(
                'male and female show all' => URL::to('tag/garment/' . $subCategory . '/all/all'),
                'male and plus size blogger' => URL::to('tag/garment/' . $subCategory . '/male/yes'),
                'male and not plus size blogger' =>  URL::to('tag/garment/' . $subCategory . '/male/no'),
                'male and not and plus size blogger' => URL::to('tag/garment/' . $subCategory . '/male/both'),
                'female and plus size blogger' => URL::to('tag/garment/' . $subCategory . '/female/yes'),
                'female and not plus size blogger' => URL::to('tag/garment/' . $subCategory . '/female/no'),
                'female and both not and plus size blogger' => URL::to('tag/garment/' . $subCategory . '/female/both'),
                'male and female plus size blogger' => URL::to('tag/garment/' . $subCategory . '/both/yes'),
                'male and female not plus size blogger' => URL::to('tag/garment/' . $subCategory . '/both/no'),
                'male and female both plus size blogger' => URL::to('tag/garment/' . $subCategory . '/both/both'),
            )
         )
      )

    @include('include.category-menu', array('menu' => array('Categories'=>Session::get('urlGarmentCategory'),'Sub Categories'=>Session::get('urlGarmentSubCategory1'), 'Items'=>URL::full()) ))
     ...
     {!! Form::open(array('route' => array('tag.garment.update', 2)   , 'method' => 'PUT', 'enctype'=>'multipart/form-data')) !!}
      ...
      <div class="portlet portlet-default">
        ...
        <div class="portlet-header">
          <h3 class="portlet-title"><u>Garment Category</u></h3>
        </div> <!-- /.portlet-header -->
                  ...
                  <tr id="garment-item-container-{{$page->id}}" >
                   ...
                   <td class="valign-middle">
                    ...
                    <td class="text-center valign-middle">
                      ...
                      &nbsp;
                       <span class="btn btn-xs btn-primary" onclick="admin_tag_delete_item('#garment-item-container-{{$page->id}}', '{{$page->id}}', 'garment', '{{route('tag.garment.destroy', $page->id)}}')" ><i class="fa fa-times"></i></span>
                    </td>
